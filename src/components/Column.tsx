...
export default function Column({ status, tasks, onEdit, onDelete, onMove }: ColumnProps) {
  ...
  return (
    <div
      ...
      className={`flex flex-col min-h-[340px] sm:min-h-[500px] rounded-[32px] transition-all duration-300 relative
                  ${isOver ? styles.dropHighlight : `${styles.bg} ${styles.border}`}`}
    >
      {/* Column Header - Pills style */}
      <div className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5">
        <div className="flex items-center gap-3">
          <div className={`px-3 sm:px-4 py-1.5 rounded-full border-2 border-black font-bold text-xs sm:text-sm ${styles.pillColor} shadow-brutal-sm`}>
      ...
      {/* Task List */}
      <div className="flex-1 p-3 sm:p-4 pt-1 space-y-3 sm:space-y-4 overflow-y-auto">
        {tasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full min-h-[180px] sm:min-h-[250px] text-center opacity-60">
            <div className="w-14 h-14 sm:w-16 sm:h-16 bg-white border-2 border-black border-dashed rounded-full flex items-center justify-center mb-4">
